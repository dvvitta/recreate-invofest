        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase text-xs font-bold tracking-wider">
              ...
              <th className="px-6 py-4">Speaker</th> {/* <-- 1. Kolom Header Speaker Ditambahkan */}
          ...
          <tbody className="divide-y divide-gray-100">
            {/* STATUS LOADING (colSpan diubah jadi 6) */}
            {isLoading && (
              <tr>
            ...
            {/* STATUS ERROR KONEKSI (colSpan diubah jadi 6) */}
            {isError && (
              <tr>
                <td colSpan={6} className="py-12 text-center text-red-600 text-sm font-medium">
                  Gagal mengambil data. Pastikan backend sudah aktif dan CORS diizinkan.
                </td>
              </tr>
            )}

            {/* JIKA DATA BERHASIL DI-FETCH & KOSONG (colSpan diubah jadi 6) */}
            ...
            {!isLoading && !isError && events?.map((event) => (
              <tr key={event.id} className="hover:bg-gray-50 transition-colors">
                ...
                {/* 2. Baris Data Speaker Baru dengan Avatar Bulat Kecil */}
                    ...
                    <div className="flex items-center gap-2.5">
                      {event.speaker.image && (
                        <img 
                          ...
                          onError={(e) => {
                            // Cadangan jika URL gambar rusak sewaktu-waktu
                            (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=50&h=50&q=80";
                          }}
                        />
                      )}
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold text-gray-800">{event.speaker.name}</span>
                        <span className="text-[10px] text-gray-400 font-normal">{event.speaker.role}</span>
                      </div>
                    </div>
                  ) : (
                    <span className="text-gray-400 text-xs italic">No Speaker</span>
                  )}
                </td>
