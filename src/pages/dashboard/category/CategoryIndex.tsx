...
import { useGetCategories, useDeleteCategory } from "../../../Hooks/useCategories"; // Sesuaikan path relative ke hooks kamu

export default function CategoryIndex() {
  // 1. Ambil data kategori & fungsi mutasi hapus dari React Query
  const { data: categories, isLoading, isError } = useGetCategories();
  const deleteCategoryMutation = useDeleteCategory();

  // 2. Handler untuk menghapus kategori
  const handleDelete = (id: number, name: string) => {
    if (window.confirm(`Apakah kamu yakin ingin menghapus kategori "${name}"?`)) {
      ...
      <div className="overflow-x-auto border border-gray-100 rounded-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {isLoading && (
              <tr>
                <td colSpan={2} className="py-12 text-center text-gray-500 text-sm">
            ...
            {isError && (
              <tr>
                <td colSpan={2} className="py-12 text-center text-red-600 text-sm font-medium">
                  Gagal mengambil data kategori dari server.
                </td>
              </tr>
            )}

            {/* JIKA DATA SUKSES DI-FETCH NAMUN KOSONG */}
            ...
            {!isLoading && !isError && categories?.map((category) => (
              <tr key={category.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 font-medium text-gray-900">
                  {category.name} 
                  <span className="text-xs text-gray-400 ml-2 font-normal">(ID: {category.id})</span>
                </td>
